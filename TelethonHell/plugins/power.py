...
async def restart(event):
    if Config.HEROKU_APP_NAME and Config.HEROKU_API_KEY:
        ...
        await eor(event, f"✅ **Restarted ʄʀǟɢɮօȶ** \n**Type** `{hl}ping` **after 1 minute to check if I am working !**")
        await event.client.disconnect()


@hell_cmd(pattern="restart$")
async def re(hell):
    event = await eor(hell, "Restarting ʄʀǟɢɮօȶ ...")
    try:
        await restart(event)
...
async def rel(event):
    await eor(event, "**Reloaded ʄʀǟɢɮօȶ!** \n\n__This might take a minute.__")
    await reload_hellbot()


@hell_cmd(pattern="shutdown$")
async def down(event):
    await eor(event, "**[ ⚠️ ]** \n**ʄʀǟɢɮօȶ is now turned off. Manually turn it on to start again.**")
...
CmdHelp("power").add_command(
    ...
    "shutdown", None, "Turns off ʄʀǟɢɮօȶ. Userbot will stop working unless you manually turn it on."
).add_command(
