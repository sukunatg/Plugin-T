...
async def muth(event):
    ...
    ForGo10God = x[0]
    if event.is_private:
        hell = await eor(event, "**Enough, muted successfully  !!**")
...
async def ban(event):
    ...
    if reason:
        await hellevent.edit(
            f"**Lol** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{event.chat.title}]` !!\n**Reason :** `{reason}`"
        )
    else:
        await hellevent.edit(
            f"**Lol** [{user.first_name}](tg://user?id={user.id}) **is now banned in**  `[{event.chat.title}]`!!"
...
async def kick(event):
    ...
    if str(user.id) in DEVLIST:
        return await eor(event, "**Bruh i can't kick my boss**")
