...
async def kk(event):
    ...
    o = "\n".join(_o)
    OUTPUT = f"""
<h1>List of Plugins in ʄʀǟɢɮօȶ:</h1>
...
"""
    hell = await telegraph_paste("All available plugins in ʄʀǟɢɮօȶ", OUTPUT)
    await eor(event, f"[All available plugins in ʄʀǟɢɮօȶ]({hell})", link_preview=False)
...
CmdHelp("core").add_command(
    "install", "<reply to a .py file>", "Installs the replied python file if suitable to ʄʀǟɢɮօȶ's codes.`\n**🚩 Flags :** `-f"
).add_command(
    "uninstall", "<plugin name>", "Uninstalls the given plugin from ʄʀǟɢɮօȶ. To get that again do .restart", "uninstall alive"
).add_command(
